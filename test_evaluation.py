            ...
            for row in ds:
                ...
                if len(answer.split()) > max_tokens:
                    if lazy_tokens:
                        ...
                        continue
                if len(answer.split()) < min_tokens:
                    continue
                # Construct prompt
                prompt = (
                    f"Question: {question}\n"
                ...
                if len(prompt.split()) > max_p_tokens:
                    if lazy_tokens:
                        prompt = ' '.join(prompt.split()[:max_p_tokens])
                    else:
                        continue
                if len(prompt.split()) < min_p_tokens:
                    continue
                dataset.append({
                    "sample_num": len(dataset) + 1,
                    "prompt": prompt,
...
async def init_async_llm_engine(
    ...
    if expert_parallel_size > 1:
        llm_args["expert_parallel_size"] = expert_parallel_size
    if distributed_executor_backend is not None and len(distributed_executor_backend) > 0:
        llm_args["distributed_executor_backend"] = distributed_executor_backend
    if max_model_len is not None:
        llm_args["max_model_len"] = max_model_len
...
async def process_example(llm: AsyncLLMEngine, example: Dict, sampling_params: SamplingParams, epoch_idx: int, idx: int):
    ...
    return generated_text, elapsed, final_output_obj
...
async def async_main_evaluate_concurrent(
    ...
    if max_samples is not None and len(dataset) > max_samples:
        ...
        print(f"Only {len(dataset)} samples out of requested {max_samples} are present!")
        exit(1)
    ...
    for epoch_idx in range(epochs):
        ...
        for idx in tqdm(range(len(results))):
            ...
            if result is None:
                continue
            generated_text, elapsed, output_obj = result
...
async def async_main(args, dataset, max_tokens):
    ...
    results = [
        ...
        f"Max Prompt Tokens:      {args.max_p_tokens}",
        f"Min Prompt Tokens:      {args.min_p_tokens}",
...
def main():
    ...
    parser.add_argument("--min_tokens", type=int, default=1000,
                        help="Minimum number of tokens to generate.")
    parser.add_argument("--max_p_tokens", type=int, default=1000,
                        help="Maximum number of tokens for prompt.")
    parser.add_argument("--min_p_tokens", type=int, default=1000,
                        help="Minimum number of tokens for prompt.")
    parser.add_argument("--lazy_tokens", type=bool, default=False,
                        help="Shape dataset to fit min/max token limits")
    ...
    parser.add_argument("--dsro", type=str, default="",
                        help="Enable DeepSeek-R1 configs.")
    parser.add_argument("--mem_util", type=float, default=0.9,
    ...
    dataset, max_tokens = load_dataset_for_eval(
        args.dataset_id,
        args.max_tokens,
        args.min_tokens,
        args.max_p_tokens,
